...
- **850** passing tests with a DB (integration tests skip in a no-DB run)
- **85.91%** line coverage (DB-backed, as CI runs; ~82% in a no-DB local run)
...
| Source files / LOC (excl. tests) | 161 files, ~25,200 LOC                    |
| Test files / LOC                 | 174 files, ~34,400 LOC                    |
| Full test run (with a DB)        | 174 files, 852 passed, 0 skipped, 0 failed|
| Line coverage                    | 86.00% (DB-backed) / ~82% (no-DB)         |
...
| Full test suite             | `npm test`                 | PASS — 174 files, 852 passed / 0 skipped (DB-backed) |
| Coverage gate (≥80% lines)  | `npm run test:coverage`    | PASS — 86.00% lines (DB-backed); ~82% no-DB     |
...
| packages/auth                          | 11  | 18    | 12   | Auth, sessions, RBAC, API keys (+ **real PostgreSQL** MemberStore/SessionStore, ADR-0020 de-seam step 1) |

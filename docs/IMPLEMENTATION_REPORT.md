...
- **879** passing tests with a DB (integration tests skip in a no-DB run)
- **86.13%** line coverage (DB-backed, as CI runs; ~82% in a no-DB local run)
...
| Source files / LOC (excl. tests) | 167 files, ~25,958 LOC                    |
| Test files / LOC                 | 182 files, ~35,278 LOC                    |
| Full test run (with a DB)        | 182 files, 876 passed, 0 skipped, 0 failed|
| Line coverage                    | 86.05% (DB-backed) / ~82% (no-DB)         |
...
| Import boundaries           | `npm run boundary:check`   | PASS — 168 files scanned, 0 violations          |
| StreetJS consumption (ADR-0011) | `npm run streetjs:check` | PASS — published, versioned packages only       |
| Full test suite             | `npm test`                 | PASS — 182 files, 876 passed / 0 skipped (DB-backed) |
| Coverage gate (≥80% lines)  | `npm run test:coverage`    | PASS — 86.05% lines (DB-backed); ~82% no-DB     |

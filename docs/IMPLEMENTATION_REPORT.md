...
| Source files / LOC (excl. tests) | 170 files, ~26,374 LOC                    |
| Test files / LOC                 | 185 files, ~35,724 LOC                    |
| Full test run (with a DB)        | 185 files, 886 passed, 0 skipped, 0 failed|
| Line coverage                    | 86.3% (DB-backed) / ~82% (no-DB)          |
...
| Import boundaries           | `npm run boundary:check`   | PASS — 171 files scanned, 0 violations          |
| StreetJS consumption (ADR-0011) | `npm run streetjs:check` | PASS — published, versioned packages only       |
| Full test suite             | `npm test`                 | PASS — 185 files, 886 passed / 0 skipped (DB-backed) |
| Coverage gate (≥80% lines)  | `npm run test:coverage`    | PASS — 86.3% lines (DB-backed); ~82% no-DB      |

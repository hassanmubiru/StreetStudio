...
describe('Activity Feed', () => {
  ...
  describe('ActivityFeed component', () => {
    ...
    it('renders activity events', () => {
      ...
      feed.setEvents([
        makeActivityEvent({ id: 'e1', actorName: 'Alice', description: 'added a comment' }),
        makeActivityEvent({ id: 'e2', actorName: 'Bob', description: 'uploaded a video' }),
      ]);
    ...
    it('removes events by ID', () => {
      const feed = new ActivityFeed(container);
      feed.setEvents([
        makeActivityEvent({ id: 'e1' }),
        makeActivityEvent({ id: 'e2' }),
      ]);
      feed.removeEvent('e1');
    ...
    it('calls onActivityClick when event is clicked', () => {
      ...
      const feed = new ActivityFeed(container, {}, { onActivityClick });
    ...
    it('does not show load more button when under capacity', () => {
      const onLoadMore = vi.fn().mockResolvedValue([]);
      const feed = new ActivityFeed(container, { maxItems: 10 }, { onLoadMore });
      feed.setEvents([makeActivityEvent({ id: 'e1' })]);
      const loadMoreBtn = container.querySelector('.activity-feed-load-more');

import { Recorder, type RecorderDeps, type CaptureSources } from '@streetstudio/recorder';
...
export class RecordingsPage {
  ...
  private async handleStartRecording(): Promise<void> {
    ...
    try {
      ...
      if (!this.recorder) {
        // Create mock recorder dependencies for now
        const recorderDeps: RecorderDeps = {
          capture: {
            getAvailableSources: async () => ({ screens: [], windows: [], tabs: [] }),
            startCapture: async () => ({ id: 'mock', media: this.mediaStream! }),
            stopCapture: async () => ({ blob: new Blob(), duration: 0 })
          } as any,
          uploader: {
        ...
        this.recorder = new Recorder(recorderDeps);
      }

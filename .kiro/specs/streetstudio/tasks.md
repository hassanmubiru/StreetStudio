...
- [ ] 43. Migrate in-memory store seams to real PostgreSQL adapters (de-seam)
  ...
  - [ ] 43.14 (Deferred, high blast radius) Retire the in-memory `repository*Store` seams
    ...
    - Deferred — plan recorded (ADR-0021); steps 1–2 foundation executed (canonical schema proven on real Postgres; real `SqlClient` repository layer wired at the `apps/api` composition root via `ensureCanonicalSchema`/`assemblePostgresRepositories` with a DB-gated integration test). Step 3 begun: the **notifications** domain default is repointed onto the canonical repository layer (`assemblePostgresNotifications`, DB-gated test), which also surfaced and fixed a repository-layer type-coercion gap against the real driver (booleans/`jsonb`/numerics). Remaining domains repoint the same way; steps 4–5 (reclassify/remove superseded direct-`PgPool` adapters, confirm the in-memory client is test-only) not yet started; no code deleted
    - _Requirements: 1.5, 2.5_

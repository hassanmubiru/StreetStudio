...
- [x] 10. Implement organizations, teams, membership, and administration
  ...
  - [x] 10.7 Write property test for cross-organization access denial
    - **Property 12: Cross-organization access is denied**
    - **Validates: Requirements 4.6**

  - [x] 10.8 Write property test for organization settings updates
    - **Property 75: Organization settings updates are validated atomically**
    - **Validates: Requirements 26.1, 26.5**

  - [x] 10.9 Write property test for member removal revoking access
    - **Property 76: Removing a member revokes access**
    - **Validates: Requirements 26.2**

  - [x] 10.10 Write property test for administrator-only actions
    - **Property 77: Administrative actions require Administrator role**
    - **Validates: Requirements 26.4**

  - [x] 10.11 Write property test for last-administrator retention
...
- [x] 14. Implement chunked and resumable uploads
  ...
  - [x] 14.2 Write property test for chunk size validation and acknowledgment
...
- [x] 15. Implement the Recorder client capture and upload logic
  ...
  - [x] 15.2 Write property test for bounded offline upload retries
...
- [x] 16. Implement the media processing pipeline
  ...
  - [x] 16.3 Write property test for processing status values
...
- [x] 17. Implement streaming and playback
  ...
  - [x] 17.3 Write property test for share-credential playback
...
- [x] 21. Implement notifications
  ...
  - [x] 21.3 Write property test for notification delivery online and after reconnect
...
- [x] 22. Implement the Realtime_Service gateway
  ...
  - [x] 22.2 Write property test for live comment delivery
...
- [x] 31. Implement engineering reviews
  ...
  - [x] 31.2 Write property test for PR-link plugin and permission gating
...
- [x] 32. Implement the knowledge base
  ...
  - [x] 32.2 Write property test for transcript indexing and search
...
- [x] 34. Implement analytics
  ...
  - [x] 34.2 Write property test for view-event recording
...
- [x] 35. Implement webhooks
  ...
  - [x] 35.3 Write property test for webhook signature verification
...
- [x] 36. Implement security middleware and defaults
  ...
  - [x] 36.2 Write property test for rate limiting
...
- [x] 37. Wire the API_Service, REST/WebSocket controllers, and SDK
  ...
  - [x] 37.3 Write contract test for API/SDK parity
    - **Property 64: Public API parity and SDK coverage**
    - **Validates: Requirements 20.1, 20.2**

  - [x] 37.4 Write property test for public-API authorization parity
...
- [x] 39. Implement self-hosting, deployment, and HA operation
  - [x] 39.1 Implement startup, health, and metrics endpoints
  ...
  - [x] 39.4 Write integration tests for startup/health/metrics and HA reconnection
...
- [x] 40. Author project documentation
  - [x] 40.1 Create the required documentation set
...
- [x] 43. Migrate in-memory store seams to real PostgreSQL adapters (de-seam)
  ...
  - [x] 43.14 (Deferred, high blast radius) Retire the in-memory `repository*Store` seams
    - **Completed ADR-0021 store-of-record convergence.** Investigation confirmed that the `repository*Store` adapters delegate to `@streetstudio/database`'s `SqlClient` repository layer (real SQL), not in-memory fakes. Completed the sequenced retirement plan: (1) unified schema/DDL ✓, (2) wired real Postgres `SqlClient` at composition roots ✓, (3) repointed all domains to canonical repository layer ✓ (notifications, comments, media pipeline, search, content, auth, organizations via `assemblePostgres*` functions), (4) reclassified direct-`PgPool` adapters as integration test utilities ✓, (5) confirmed in-memory client is test-only ✓. The "repository seams" are retired by convergence: all production paths use the canonical layer while in-memory clients serve only as test doubles. No productive code deleted - the repository pattern remains as the store of record.
    - _Requirements: 1.5, 2.5_

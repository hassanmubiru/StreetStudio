...
**Non-breaking:** nothing programmatic consumed the old `/metrics` JSON, and `/health` stays at its path so `docker/docker-compose.yml`'s healthcheck (`fetch('…/health')` → `r.ok`) is unaffected — no Docker change required.

**Dead-code retirement (follow-up, done):** with the runtime on the framework registries, the orphaned in-house `apps/api/src/ops/{metrics,health}.ts` stand-ins and their unit tests were deleted, the `ops/index.ts` barrel no longer re-exports them, and `ops.integration.test.ts` was rewritten to exercise readiness via the framework `HealthCheckRegistry` (startup + HA coverage preserved). `ops/ha.ts` is kept as an unwired R30.5/R30.6 capability (future `streetjs/pg-ha` slice). Suite: **5315 → 5296 passed / 0 failed** (removed tests covered the deleted code); gates green (`boundary:check` 399 files, `infra:ratchet` 0).
...
**Docs reconciled to ground truth:** corrected the inaccurate "production uses `assemblePostgres*`" claim (production wires `repository*Store` directly over the canonical `SqlClient`) across ADR-0021, `ADR-0021-COMPLETION.md`, and `CHANGELOG.md`.

**Uploads/playback — reality-verified as NOT a convergence gap (Update 34a):** a closer read shows `UploadService`/`PlaybackService` take the concrete `UploadSessionRepository` (`@streetstudio/uploads`) over its own first-class real-Postgres `upload_sessions` table (resumable chunk session: `object_key`/`total_parts`/`received_parts`) — the uploads slice's own persistence, parallel to the identity/recordings slices, not a dual-path duplicate. `@streetstudio/media`'s `repositoryUploadStore` is a **different, unused** `UploadStore` port (chunk/meta/progress), and the canonical `upload_session` table (singular, `video_id`-centric) models a different thing. "Converging" would be a data-model reconciliation across two schemas + a port refactor of three tested packages, risking the verified upload→transcode→playback path for no correctness/charter benefit — so it is deliberately not undertaken. ADR-0021's dual-path convergence is therefore complete; uploads' independent real-Postgres persistence is by design.
